extension Tags.Input {
  ...
  public static func password(name: String, value: String = "", attributes extraAttrs: [Attribute] = []) -> NodeConvertible {
    let attributes: [Attribute] = extraAttrs + [Attributes.InputType(.Password), Attributes.Name(name), Attributes.Value(value)]
    return Tags.Input(attributes)
  }
}
